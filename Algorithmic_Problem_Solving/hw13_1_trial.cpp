...
int main(){
    ...
    vii dp(n+1, vi(1001, 0));
    dp[0][0] = 1;
    FOR(i, 1, n+1){
        FOR(j, 0, 1001){
            FOR(k, 0, 1001){
                if( j - v[i]*k >= 0 ) dp[i][j] = (dp[i][j] + dp[i-1][j - v[i]*k]) % MOD;
                else break;
            }
        }
    }
    ...
    //sort(v.begin(), v.end());
    vii dpk(n+1, vi(1001, 0));
    dpk[0][0] = 1;

    FOR(i, 1, n+1){ // only choose coins index < i
        FOR(k, 0, 1001){ // number of coins chosen
            FOR(j, 0, 1001){
    ...
    FOR(k, 1, 11){
        FOR(s, 1, 11){
            FOR(i, 1, n+1){ // iterate through v
                if( s - v[i] >= 0 ) dpk[s][k] = (dpk[s][k] + dpk[s-v[i]][k-1])%MOD;
                else break;
            }
        }
    }
