...
typedef unordered_map<int, unordered_set<pair<int, char>>> umi;
...
class Vertex{
    Vertex::Vertex(int y_, int d_, char e_){
        y = y_;
        d = d_;
        e = e_;
    }
...
int main(){
    ...
    char c;
    cin >> n >> m >> k;
    unordered_map<int, unordered_set<Vertex>> graph(n, unordered_set<Vertex>());

    FOR(i, 0, m){
        cin >> x >> y >> d >> c;
        graph[x].insert(Vertex(y, d, c));
    }
