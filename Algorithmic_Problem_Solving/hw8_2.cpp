...
double distance(double t, vi& p, vi&s){
    double left = INF, right = -INF;
    FOR(i, 0, p.size() ){
        left = min( (double)p[i] + s[i]*t, left);
        right = max( (double)p[i] + s[i]*t, right);
    }
    return right - left;
}

double ternarySearch(double l, double r, vi& p, vi& s){
    double d1, d2;
    while ( r - l > EPS ){
        double m1 = l + (r-l)/3;
        double m2 = r - (r-l)/3;
...
int main(){
    ...
    double ans = ternarySearch(0, 200000, p, s);
    printf("%.15f\n", ans);
