...
void getmin(vi& final, vi& left, vi& right){
    auto it1 = left.begin();
    auto it2 = right.begin();
    while( (it1 != left.end() || it2 != right.end()) && final.size() < 5 ){
...
void build(vii&val, int k, int nl, int nr) {
    if (nl == nr) {
    ...
    getmin(val[k], val[2*k+1], val[2*k+2]);
}

// update arr[x] to v
void update(vii& val, int k, int nl, int nr, int x, int v) {
...
vector<int> get(vii& val, int k, int nl, int nr, int l, int r) {
...
int main(){
    ...
    FOR(i, 0, q){
        cin >> op >> l >> r;
        if( op == 'q' ){
            ans = get(val, 0, 1, n, l, r);
            printf("%d\n", ans[0]);
        }
        if( op == 'c' ) update(val, 0, 1, n, l, r);
        if( op == 'a' ){
            cin >> k;
            vi ans = get(val, 0, 1, n, l, r);
            //pv(ans);
            ll total = 0;
            int count = 0;
            FORIT(it, ans){
