...
int binarySearch(FenwickTree& ft, int x, int l, int r){

    while( r - l > 1){
        int m = (l+r)/2;
        //print("m: ", m, "rsq: ", ft.rsq(m));
        if( ft.rsq(m) < x ) l = m;
        else r = m;
    }

    if( ft.rsq(l) == x ) return l;
...
int main(){
    ...
    FenwickTree ft(500000);
    vi array;
    array.pb(-1);
    array.pb(-1); // starts from 2
    ft.adjust(1, 1);
    int last = 1; // keep track of how many numbers actually in array
    ...
    FOR(i, 0, n){
        ...
        if( op == 'a' ){
            ...
            if( last != 0 ){
                //printf("last %d\n", last);
                //int ind = binarySearch(ft, last, 1, (int)array.size());
                //print("add to ind", last+1);
                ft.adjust(last+1, 1);
            }
            last ++;
        }
        else if( op == 'p' ){
            int ind = binarySearch(ft, x+2, 1, (int)array.size());
            //print("ind ", ind, "x ", x);
            printf("%d\n", array[ind]);
        }
        else if( op == 'd' ){
