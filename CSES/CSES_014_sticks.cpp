/* prove that the minimum equal length has to be at the median */
...
int main(){
    ...
    midleft = sticks[(n-1)/2];
    midright = sticks[(n-1)/2+1];

    ll ans = min(cost(sticks, midleft), cost(sticks, midright) );
