/* binary search, cannot use vector since deletion takes too much time. almost same as 012 */
...
int main(){
    ...
    FOR(i, 0, n){
        scanf("%d", &a);
        auto it = tickets.find(a);
        if( it == tickets.end() ) tickets[a] = 1;
        else it->S += 1;
    }

    FOR(i, 0, m){
        scanf("%d", &a);
        auto upper = tickets.upper_bound(a);
        if( upper !=tickets.begin() ){
            upper --;
            printf("%d\n", upper->F);
            upper->S --;
            if( upper->S == 0 ) tickets.erase(upper);
        }
        else printf("-1\n" );
    }
