...
int main(){
    ...
    auto hit = houses.begin();
    auto pit = persons.begin();

    int count = 0;

    while( hit != houses.end() && pit != persons.end() ){
        if( *pit >= hit->F && *pit <= hit->S ){
            count ++;
            pit ++;
            hit ++;
        }
        else if( *pit < hit->F ) pit ++;
        else if( *pit > hit->S ) hit ++;
    }
