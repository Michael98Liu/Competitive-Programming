/* greedy, binary search. Show that the vector of tower height has to be monotonically increasing.
...
int binarySearch(vi& v, int k){
    // find index of smallest element that is larger than k
    int l = 0;
    int r = v.size();
...
int main(){
    ...
    FOR(i, 0, n){
        cin >> k;
        if( towers.size() == 0 || (towers.size() != 0 && k >= *towers.rbegin()) ) towers.pb(k);
        else{
            int ind = binarySearch(towers, k);
            towers[ind] = k;
        }
    }
