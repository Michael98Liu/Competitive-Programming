...
#define contains(m,x) find(m.begin(), m.end(), x) != m.end()
    ...
    if( a.size() != b.size() ){
        ...
        FOR(i, 0, a.size() ){
            if( a[i] == b[i] ) continue;
                ...
                if( contains(vowels, a[i]) && contains(vowels, b[i]) ) continue;
                else if( !(contains(vowels, a[i])) && !(contains(vowels, b[i])) ) continue;
                else{
                    printf("No\n" );
                    return 0;
                }
